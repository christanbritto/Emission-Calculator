...
const BiofuelFeature: React.FC = () => {
  ...
  useEffect(() => {
    if (biofuel.mass > 0) {
      ...
      setBiofuel(prev => ({ ...prev, lcv_kg: calculatedLcv }));
    }
  ...
  const simResults = useMemo(() => {
    ...
    const baselineCO2 = (simHFO * hfoCf) + (simLFO * lfoCf) + (simMGO * mgoCf);
    ...
    const fossilEnergy = (simFossilMass * 1_000_000) * 0.041;
    const totalEnergy = bioEnergy + fossilEnergy;
    const bioRatio = totalEnergy > 0 ? bioEnergy / totalEnergy : 0;
    const simBlendedCf = (bioRatio * results.biofuel.cf) + ((1 - bioRatio) * avgFossilCf);
  ...
  const comparisonData = useMemo(() => {
    ...
    return scenarios.map(s => ({ ...s, totalCO2: s.cf * totalMass }));
  }, [results, totalMass]);

  const insights = useMemo(() => {
    const mgoCf = FOSSIL_FUEL_STANDARDS[FossilFuelType.MGO].cf;
    const reductionVsMGO = mgoCf > 0 ? ((mgoCf - results.total.blendedCf) / mgoCf) * 100 : 0;
    const co2SavedVsMGO = (mgoCf * totalMass) - (results.total.blendedCf * totalMass);
    return { reductionVsMGO, co2SavedVsMGO, intensity: results.total.blendedCf };
  }, [results.total.blendedCf, totalMass]);

  // --- HANDLERS ---
  const handleBioChange = (field: keyof BiofuelData, value: string | boolean) => {
    setBiofuel(prev => ({ ...prev, [field]: typeof value === 'string' ? parseFloat(value) || 0 : value }));
  };
  const handleFossilChange = (field: keyof FossilData, value: string) => {
    setFossil(prev => ({ ...prev, [field]: field === 'mass' ? parseFloat(value) || 0 : value }));
  };
  const handleTotalMassChange = (newTotalStr: string) => {
    const newTotal = parseFloat(newTotalStr) || 0;
    if (newTotal === 0) { setBiofuel(prev => ({ ...prev, mass: 0 })); setFossil(prev => ({ ...prev, mass: 0 })); return; }
  ...
  const applyGradePreset = (grade: BiofuelGrade) => {
    let p = 0;
    if (grade === BiofuelGrade.B24) p = 0.24; else if (grade === BiofuelGrade.B30) p = 0.30;
    else if (grade === BiofuelGrade.B50) p = 0.50; else if (grade === BiofuelGrade.B100) p = 1.00;
    const currentTotal = totalMass || 100;
    setBiofuel(prev => ({ ...prev, mass: currentTotal * p }));
    setFossil(prev => ({ ...prev, mass: currentTotal - (currentTotal * p) }));
  };

  const formatNum = (num: number, min: number, max: number) => num.toLocaleString(undefined, { minimumFractionDigits: min, maximumFractionDigits: max });
  ...
  const downloadReport = async () => {
    ...
    } catch (e) { console.error(e); } finally { reportEl.style.display = 'none'; setIsGeneratingPDF(false); }
  };

  const EmissionGraph = ({ isFull = false, isReport = false }) => {
    const chartHeight = 320; const chartWidth = 850; const paddingX = 80; const paddingY = 60;
    const ah = chartHeight - paddingY * 2; const aw = chartWidth - paddingX * 2;
    const allCfs = comparisonData.map(d => d.cf);
    const minCf = Math.min(...allCfs) * 0.95; const maxCf = Math.max(...allCfs) * 1.05;
    const getY = (cf: number) => chartHeight - paddingY - ((cf - minCf) / (maxCf - minCf)) * ah;
    const getX = (index: number) => paddingX + (index * (aw / (comparisonData.length - 1)));
    const lp = comparisonData.map((d, i) => `${i === 0 ? 'M' : 'L'} ${getX(i)} ${getY(d.cf)}`).join(' ');
    const ap = `M ${getX(0)} ${chartHeight - paddingY} ${comparisonData.map((d, i) => `L ${getX(i)} ${getY(d.cf)}`).join(' ')} L ${getX(comparisonData.length - 1)} ${chartHeight - paddingY} Z`;

    return (
      <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className={`w-full overflow-visible ${isFull ? 'h-full max-h-[70vh]' : 'mx-auto'}`}>
        <defs>
          <linearGradient id="areaGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#3b82f6" stopOpacity="0.15" /><stop offset="100%" stopColor="#3b82f6" stopOpacity="0" /></linearGradient>
          <linearGradient id="lineGradient" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#ef4444" /><stop offset="50%" stopColor="#f97316" /><stop offset="100%" stopColor="#3b82f6" /></linearGradient>
        </defs>
        {[0, 0.25, 0.5, 0.75, 1].map(v => (
          <g key={v}>
            <line x1={paddingX} y1={paddingY + v * ah} x2={chartWidth - paddingX} y2={paddingY + v * ah} stroke="currentColor" className={isReport ? "text-slate-200" : "text-slate-100 dark:text-slate-800"} />
            <text x={paddingX - 15} y={paddingY + v * ah + 4} textAnchor="end" className="fill-slate-400 text-[9px] font-mono">{(maxCf - v * (maxCf - minCf)).toFixed(2)}</text>
          </g>
        ))}
        <path d={ap} fill="url(#areaGradient)" />
        <path d={lp} fill="none" stroke="url(#lineGradient)" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
        {comparisonData.map((d, i) => (
          <g key={d.name}>
            <circle cx={getX(i)} cy={getY(d.cf)} r={d.isCurrent ? "10" : "7"} fill={d.color} stroke="#fff" strokeWidth="2" />
            <text x={getX(i)} y={getY(d.cf) - 25} textAnchor="middle" className={`text-[14px] font-black font-mono ${d.isCurrent ? 'fill-blue-600' : 'fill-slate-900'}`}>{formatNum(d.totalCO2, 1, 1)}</text>
            <text x={getX(i)} y={chartHeight - paddingY + 30} textAnchor="middle" className={`text-[10px] font-black uppercase tracking-[0.1em] ${d.isCurrent ? 'fill-blue-600' : 'fill-slate-500'}`}>{d.name}</text>
          </g>
        ))}
      </svg>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-end mb-4">
        <button onClick={downloadReport} disabled={isGeneratingPDF} className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 text-white rounded-xl transition-all shadow-lg font-bold uppercase text-[10px] tracking-widest">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 xl:col-span-3 space-y-6">
          <section className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex items-center gap-2 mb-4"><Layers className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /><h3 className="font-bold text-lg dark:text-white">Grade Presets</h3></div>
            <div className="grid grid-cols-2 gap-2 mb-4">
              {[BiofuelGrade.B24, BiofuelGrade.B30, BiofuelGrade.B50, BiofuelGrade.B100].map(grade => (
                <button key={grade} onClick={() => applyGradePreset(grade)} className="py-2.5 px-3 text-xs font-bold border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl hover:border-blue-400 hover:text-blue-600 transition-all">Apply {grade}</button>
              ))}
            </div>
          ...
          <section className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex items-center gap-2 mb-4 text-slate-600 dark:text-slate-400"><CalcIcon className="w-5 h-5" /><h3 className="font-bold text-lg text-slate-900 dark:text-white">Delivery Volume</h3></div>
          ...
          <section className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex items-center gap-2 mb-6 text-slate-600 dark:text-slate-400"><Settings2 className="w-5 h-5" /><h3 className="font-bold text-lg text-slate-900 dark:text-white uppercase tracking-tighter">Mass Components</h3></div>
        ...
        <div className="lg:col-span-8 xl:col-span-9 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className={`p-10 rounded-[2.5rem] border shadow-lg flex flex-col justify-between transition-all ${results.biofuel.isSustainabilityCompliant ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/50' : 'bg-rose-50 dark:bg-rose-950/20 border-rose-200 dark:border-rose-900/50'}`}>
              <div>
                ...
                <h4 className="text-3xl font-black mb-3 text-slate-800 dark:text-slate-100 tracking-tight">{results.biofuel.isSustainabilityCompliant ? 'Verified Compliant' : 'Non-Compliant Batch'}</h4>
                <p className="text-base text-slate-600 dark:text-slate-300 font-medium leading-relaxed">{results.biofuel.isSustainabilityCompliant ? `Calculation follows GHG intensity correction guidelines. Corrected C_f: ${formatNum(results.biofuel.cf, 3, 3)}` : `Threshold exceeded. Biofuel emissions default to Fossil C_f: ${formatNum(results.fossil.cf, 3, 3)}`}</p>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
            <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
              <h3 className="font-bold text-slate-800 dark:text-slate-100 text-base flex items-center gap-3"><Settings2 className="w-5 h-5 text-indigo-500" />Technical Reporting Ledger</h3>
            </div>
            <div className="w-full overflow-x-auto">
              <table className="w-full text-xs xl:text-sm text-left border-collapse table-fixed min-w-[800px]">
                  <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-500 font-bold uppercase tracking-widest text-[10px]">
                    <tr className="border-b border-slate-100 dark:border-slate-800"><th className="px-8 py-5">Component</th><th className="px-4 py-5 text-center">Energy [MJ]</th><th className="px-4 py-5 text-center">Energy %</th><th className="px-4 py-5 text-center">C<sub>f</sub></th><th className="px-8 py-5 text-right">Contribution</th></tr>
                  </thead>
                  ...
                  <tfoot className="bg-slate-900 dark:bg-black text-white font-bold border-t border-slate-800">
                    <tr><td className="px-8 py-8 text-sm uppercase tracking-widest">Total Weighted Blend</td><td colSpan={3}></td><td className="px-8 py-8 text-right text-3xl font-black text-blue-400 tabular-nums">{formatNum(results.total.blendedCf, 3, 3)}</td></tr>
                  </tfoot>
              </table>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm p-8 flex flex-col">
              ...
              <div className="mt-auto p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-800/50">
                <div className="flex items-start gap-4">
                  <div className="bg-blue-600 p-2 rounded-lg mt-1 shrink-0"><TrendingDown className="w-4 h-4 text-white" /></div>
                  <div>
                    <h4 className="font-black text-[10px] text-blue-600 uppercase tracking-[0.2em] mb-2">Impact Analysis</h4>
                    <p className="text-xs xl:text-sm text-slate-600 dark:text-slate-300 font-medium leading-relaxed">Current blend achieves a <span className="text-blue-600 font-bold">{formatNum(insights.reductionVsMGO, 1, 1)}%</span> reduction in CO<sub>2</sub> intensity ($C_f$: {formatNum(insights.intensity, 3, 3)}) compared to standard Marine Gas Oil.</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-8 shadow-sm flex flex-col justify-between transition-all">
              <div className="flex items-center justify-between mb-8"><h3 className="font-bold text-slate-800 dark:text-slate-100 flex items-center gap-3 uppercase tracking-tight"><Sparkles className="w-5 h-5 text-indigo-500" /> WHAT-IF SIMULATION</h3><span className="bg-indigo-600 text-[10px] font-black text-white px-3 py-1 rounded-full uppercase tracking-widest shadow-lg shadow-indigo-500/30">Fleet Scale-Up</span></div>
              <div className="grid grid-cols-3 gap-4 mb-8">
                <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">HFO (MT)</label><input type="number" value={simHFO} onChange={(e) => setSimHFO(parseFloat(e.target.value) || 0)} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 font-mono font-bold text-lg focus:ring-2 focus:ring-indigo-500 transition-all outline-none" /></div>
                <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">LFO (MT)</label><input type="number" value={simLFO} onChange={(e) => setSimLFO(parseFloat(e.target.value) || 0)} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 font-mono font-bold text-lg focus:ring-2 focus:ring-indigo-500 transition-all outline-none" /></div>
                <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">MGO (MT)</label><input type="number" value={simMGO} onChange={(e) => setSimMGO(parseFloat(e.target.value) || 0)} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 font-mono font-bold text-lg focus:ring-2 focus:ring-indigo-500 transition-all outline-none" /></div>
              </div>
              <div className="mb-8 space-y-4">
                <div className="flex items-center justify-between"><span className="text-[11px] font-black text-indigo-500 uppercase flex items-center gap-2 tracking-widest"><Droplets className="w-4 h-4" /> Biofuel Displacement</span><span className="text-xl font-black font-mono text-slate-800 dark:text-white">{simBioPercent}%</span></div>
              ...
              <div className="bg-emerald-500 p-8 rounded-[2rem] text-white shadow-2xl shadow-emerald-500/30 flex items-center justify-between">
                <div><span className="text-[11px] font-black uppercase opacity-80 block mb-1 tracking-widest">Total Carbon Savings</span><span className="text-4xl font-black font-mono">-{formatNum(simResults.savings, 1, 1)} <span className="text-lg opacity-60">MT</span></span></div>
                <div className="text-right"><span className="text-3xl font-black block leading-none">{simResults.reductionPercent.toFixed(1)}%</span><span className="text-[10px] font-black uppercase tracking-widest opacity-80">Reduction</span></div>
              </div>
            </section>
          </div>
        </div>
      ...
      {isGraphMaximized && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-12 bg-slate-900/90 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 w-full h-full max-w-7xl rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 p-12 overflow-y-auto"><EmissionGraph isFull /></div>
          </div>
        </div>
      )}

      {/* PDF TEMPLATE */}
      <div ref={reportTemplateRef} style={{ display: 'none', width: '1200px', padding: '60px', backgroundColor: '#ffffff', color: '#0f172a' }} className="font-['Inter']">
        <div className="flex justify-between items-start border-b-4 border-slate-900 pb-10 mb-10">
          <div>
            <div className="flex items-center gap-4 mb-4"><div className="bg-blue-600 p-3 rounded-xl"><Anchor className="w-10 h-10 text-white" /></div><h1 className="text-5xl font-black tracking-tighter uppercase">MARITIME COMPLIANCE REPORT</h1></div>
            <p className="text-slate-400 text-lg font-bold uppercase tracking-widest">Emission Factor Verification • MEPC.1/Circ.905</p>
          </div>
          <div className="text-right"><div className="flex items-center justify-end gap-2 text-slate-500 font-bold mb-2 uppercase text-sm"><Calendar className="w-4 h-4" /> {new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</div><div className="flex items-center justify-end gap-2 text-slate-900 font-black uppercase text-base"><Building2 className="w-5 h-5" /> COMPLIANCE DEPT</div></div>
        </div>
        <div className="grid grid-cols-2 gap-10 mb-12">
          <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white"><h2 className="text-blue-400 text-sm font-black uppercase tracking-widest mb-6 border-b border-blue-400/20 pb-4">Consolidated Result</h2><div className="flex items-baseline gap-4"><span className="text-[120px] font-black leading-none">{formatNum(results.total.blendedCf, 3, 3)}</span><span className="text-2xl text-slate-400 font-mono">gCO<sub>2</sub>eq/g</span></div></div>
          <div className="bg-slate-50 border-2 border-slate-100 rounded-[2.5rem] p-10 flex flex-col justify-between"><div><h2 className="text-slate-500 text-sm font-black uppercase tracking-widest mb-6 border-b border-slate-200 pb-4">Sustainability Assessment</h2><div className="flex items-center gap-6 mb-4">{results.biofuel.isSustainabilityCompliant ? <ShieldCheck className="w-16 h-16 text-emerald-600" /> : <ShieldAlert className="w-16 h-16 text-rose-600" />}<h3 className="text-4xl font-black uppercase tracking-tight">{results.biofuel.isSustainabilityCompliant ? 'VERIFIED COMPLIANT' : 'NON-COMPLIANT'}</h3></div></div></div>
        </div>
        <div className="mb-12"><h2 className="text-slate-900 text-2xl font-black uppercase tracking-tighter mb-6 flex items-center gap-3"><div className="w-6 h-6 rounded bg-indigo-600" /> Technical Data Ledger</h2><table className="w-full text-lg border-collapse rounded-3xl overflow-hidden shadow-sm"><thead><tr className="bg-slate-900 text-white text-left uppercase text-xs font-black tracking-widest"><th className="p-8 border-r border-slate-800">Source Component</th><th className="p-8 border-r border-slate-800 text-center">Energy [MJ]</th><th className="p-8 border-r border-slate-800 text-center">Mass [MT]</th><th className="p-8 text-right">Component C<sub>f</sub></th></tr></thead><tbody className="bg-white border-2 border-slate-100"><tr className="border-b-2 border-slate-50"><td className="p-8 font-black flex items-center gap-3"><Droplets className="w-6 h-6 text-blue-500" /> BIOFUEL</td><td className="p-8 text-center font-mono">{formatNum(results.biofuel.energy, 0, 0)}</td><td className="p-8 text-center font-mono">{formatNum(biofuel.mass, 3, 3)}</td><td className="p-8 text-right font-black text-blue-600 font-mono">{formatNum(results.biofuel.cf, 3, 3)}</td></tr><tr><td className="p-8 font-black flex items-center gap-3"><Flame className="w-6 h-6 text-orange-500" /> {fossil.type} (BASE)</td><td className="p-8 text-center font-mono">{formatNum(results.fossil.energy, 0, 0)}</td><td className="p-8 text-center font-mono">{formatNum(fossil.mass, 3, 3)}</td><td className="p-8 text-right font-black text-orange-600 font-mono">{formatNum(results.fossil.cf, 3, 3)}</td></tr></tbody></table></div>
        <div className="mt-auto border-t-2 border-slate-200 pt-10 text-slate-400 text-sm italic"><p className="mb-6">This report represents an energy-weighted calculation of the blended emission factor (Cf) based on MEPC.1/Circ.905 guidelines.</p></div>
      </div>
    </div>
  );
};

export default BiofuelFeature;
